...
//! book. This code enhances raster_12 with bilinear filtering and mipmapping.
//!
...
/// User-selectable choice for nearest neighbor or bilinear texture mapping.
#[derive(Copy, Clone, Debug, PartialEq)]
enum TextureMapping {
    Nearest,
    Bilinear,
...
/// User-selectable choice for using mipmapping.
#[derive(Copy, Clone, Debug, PartialEq)]
enum TextureMipmap {
    NoMipmap,
    Mipmap,
...
/// Data defining the position of a single mipmap within a texture file containing a set of
/// mipmaps. Height is not recorded as this simplistic implementation of mipmaps requires it to be
/// equal to `width`.
#[derive(Clone, Copy, Debug, PartialEq)]
struct MipMap {
    start_u: usize,
...
/// A texture, represented as a set of mipmaps. Each mipmap must be square, i.e, width == height.
/// The size of a mipmap need not be a power of two, e.g., a mipmap of 600x600 texels is allowed.
//
// The `Texture` stores all mipmaps as a single data set in `texels`. `mipmaps` then indexes each
// mipmap into the correct positions within this data set.
...
impl Texture {
    /// Creates and returns a new `Texture` given the name of a texture file and details of the
    /// mipmaps within that file. `filename` is relative to the location the executable is run
    /// from. The file must be a JPEG file that uses the RGB24 format. `mipmap_widths` is a vector
    /// of the widths of each of the mipmaps contained within the file, ordered from the widest
    /// to the narrowest. Each mipmap must be square, i.e., each mipmap's height must equal its
    /// width. Each mipmap should be half the width of the previous mipmap.
    ///
    /// # Panics
    ///
    ...
    /// Panics if the width of the file does not match the data in `mipmap_widths`.
    pub fn new_from_rgb24_file(filename: &str, mipmap_widths: Vec<usize>) -> Self {
        ...
        let image_of_all_mipmaps = decoder.decode()
            .expect(&format!("Texture `{}` failed to decode", filename));
        ...
        for b in image_of_all_mipmaps.chunks(3) {
        ...
        if ((mipmap_widths.len() == 1) & (metadata.width != metadata.height)) |
            ((mipmap_widths.len() > 1) & (metadata.width as usize !=
        ...
        for (i, mm_width) in mipmap_widths.iter().enumerate() {
            mipmaps.push(MipMap {
                start_u: current_u,
                start_v: current_v,
                width: *mm_width,
            ...
            if i == 0 {
                ...
                current_v += *mm_width;  // Safe because each mipmap is square, so width == height.
            }
        }

        Self {
            texture_width: metadata.width as usize,
            texels: texels,
            mipmaps: mipmaps,
        }
    }
    ...
    /// Returns the `Rgb` color at the given `u` and `v` coordinates, expressed as `i32`s, in the
    /// given `mipmap level`. The range of `u` is 0 to width-1, `v` is 0 to height-1. 0, 0 is the
    /// top-left of the texture.
    ///
    /// # Panics
    ///
    /// Panics if `u` is not in the range 0 to width-1, `v` is not in the range 0 to height-1, or
    /// both.
    /// Panics if `mipmap_level` is invalid.
    pub fn get_texel_ints(&self, mipmap_level: usize, u: i32, v: i32) -> Rgb {
        let mipmap = self.mipmaps.get(mipmap_level).unwrap();

        if (u < 0) | (u >= mipmap.width as i32) | (v < 0) | (v >= mipmap.width as i32) {
            panic!("Tried to access a texel with an index outside the allowable range");
        }

        let mapped_u = mipmap.start_u + u as usize;
        let mapped_v = (mipmap.start_v + v as usize) * self.texture_width;

        self.texels.get(mapped_u + mapped_v).unwrap().clone()
    }


    /// Returns the `Rgb` color at the given `u` and `v` coordinates in the given given
    /// `mipmap_level`. The ranges of `u` and `v` are between 0.0 and 1.0, with 0.0, 0.0 being the
    /// top-left of the texture.
    ///
    /// # Panics
    ///
    ...
    /// Panics if `mipmap_level` is invalid.
    pub fn get_texel(&self, mipmap_level: usize, u: f64, v: f64, bilinear: bool) -> Rgb {
        if (u < 0.0) | (u > 1.0) | (v < 0.0) | (v > 1.0) {
            panic!("Tried to access a texel with a coordinate outside the range 0.0 to 1.0");
        }

        let width = self.mipmaps.get(mipmap_level).unwrap().width;
        let height = width;

        let u_exact = u * (width-1) as f64;
        let v_exact = v * (height-1) as f64;  // All mipmaps are required to be square

        if bilinear {
            let u_fract = u_exact.fract();
            let u_left = i32::max(u_exact.floor() as i32, 0);
            let u_right = i32::min(u_exact.ceil() as i32, (width-1) as i32);

            let v_fract = v_exact.fract();
            let v_top = i32::max(v_exact.floor() as i32, 0);
            let v_bottom = i32::min(v_exact.ceil() as i32, (height-1) as i32);

            let top_left_color = self.get_texel_ints(mipmap_level, u_left, v_top);
            let top_right_color = self.get_texel_ints(mipmap_level, u_right, v_top);
            let bottom_left_color = self.get_texel_ints(mipmap_level, u_left, v_bottom);
            let bottom_right_color = self.get_texel_ints(mipmap_level, u_right, v_bottom);

            let top_color = top_left_color.multiply_by(1.0 - u_fract)
                .add(&top_right_color.multiply_by(u_fract));

            let bottom_color = bottom_left_color.multiply_by(1.0 - u_fract)
                .add(&bottom_right_color.multiply_by(u_fract));

            return top_color.multiply_by(1.0 - v_fract)
                .add(&bottom_color.multiply_by(v_fract));
        } else {
            return self.get_texel_ints(mipmap_level, u_exact.round() as i32,
                v_exact.round() as i32);
        }
    }


    /// Returns the best mipmap levels given the `width` and `height` of the triangle being
    /// rendered. The larger of `width` and `height` are compared to the mipmap widths to find
    /// the mipmap levels just larger and just smaller. Both levels are returned as a tuple, the
    /// larger first. If only one mipmap level is defined, it is returned as both larger and
    /// smaller. If the triangle width/height exceeds the largest mipmap size, both larger and
    /// smaller index the largest mipmap. If the triangle width/height is smaller than the
    /// smallest mipmap size, both larger and smaller index the smallest mipmap.
    pub fn nearest_mipmap_levels(&self, width: f64, height: f64) -> (usize, usize) {
        let mipmap_count = self.mipmaps.len();
        let larger = f64::max(width, height).round() as usize;

        if larger > self.mipmaps[0].width {
            return (0, 0);
        }

        if larger < self.mipmaps[mipmap_count-1].width {
            return (mipmap_count-1, mipmap_count-1);
        }

        for (index, mipmap) in self.mipmaps.iter().enumerate() {
            if larger >= mipmap.width {
                return (index-1, index);
            }
        }

        (mipmap_count-1, mipmap_count-1)
    }
}
...
fn parse_command_line_arguments() -> Result<UserChoices, ()>{
    ...
    for arg in args {
        match arg.to_lowercase().as_str() {
            ...
            "nearest" => arg_texture_map = TextureMapping::Nearest,
    ...
    for y in p0.y.round() as i32 .. p2.y.round() as i32 {
        ...
        for x in xl.round() as i32 .. xr.round() as i32 {
            ...
            if depth_buffer.check_set_nearer_pixel(x as i32, y as i32, inv_z) {
                ...
                // If user has chosen to use mipmapping, determine the height and width of the
                // triangle after perspective projection, and use these dimensions to choose the
                // best mipmap level.
                let mm_level;
                match user_choices.texture_mipmap {
                    TextureMipmap::NoMipmap => {
                        mm_level = 0;
                    },
                    TextureMipmap::Mipmap => {
                        let tri_height = p2.y - p0.y;
                        let tri_width = f64::abs(p2.x - p0.x).max(f64::abs(p2.x - p1.x))
                            .max(f64::abs(p1.x - p0.x));
                        let nearest = triangle.texture.nearest_mipmap_levels(tri_width,
                            tri_height);
                        mm_level = nearest.0;
                    }
                }
                ...
                match user_choices.texture_perspective {
                    TexturePerspective::Linear => {
                        texel = triangle.texture.get_texel(
                            ...
                            tuscan[x_minus_xl].1,
                            tvscan[x_minus_xl].1,
                            user_choices.texture_mapping == TextureMapping::Bilinear
                        );
                    },
                    TexturePerspective::Perspective => {
                        texel = triangle.texture.get_texel(
...
fn main() {
    ...
    let mut canvas = Canvas::new("Raster 12 plus bilinear filtering and mipmapping (based on
    ...
    let instances = [
        ...
        ModelInstance::new(
            &cube,
            Vector4::new(-4.0, 2.0, 12.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(-1.0, 7.0, 18.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(7.0, 7.0, 25.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(-8.0, -8.0, 35.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(6.0, 6.0, 50.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(25.0, 3.0, 80.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(-6.0, 3.0, 100.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
        ModelInstance::new(
            &cube,
            Vector4::new(12.0, -10.0, 200.0, 1.0),
            Matrix4x4::identity(),
            1.0,
        ),
    ];
    ...
    let camera = Camera {
        position: Vector4::new(-2.0, 1.5, 0.0, 1.0),
        orientation: Matrix4x4::new_oy_rotation_matrix(-10.0),
        clipping_planes: [
    ...
    let lights = [
        Light::Ambient(AmbientLightEntity { intensity: 0.5, } ),
