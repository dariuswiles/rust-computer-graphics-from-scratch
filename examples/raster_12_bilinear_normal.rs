//! Implementation based on chapters 14 and 15 of Gabriel Gambetta's
//! [Computer Graphics from Scratch](https://gabrielgambetta.com/computer-graphics-from-scratch/)
//! book. I am not affiliated with Gabriel or his book in any way.
//!
//! This code enhances raster_12 with bilinear filtering and normal mapping.
...
const TEXTURE_FILENAME: &str = "medieval_wood_diff_1k.jpg";
const TEXTURE_NORMAL_FILENAME: &str = "medieval_wood_nor_gl_1k.jpg";
...
impl Texture {
    ...
    /// Returns the `Rgb` color at the given `u` and `v` coordinates, expressed as `i32`s. The
    /// range of `u` is 0 to width-1, `v` is 0 to height-1. 0, 0 is the top-left of the texture.
    ///
    /// # Panics
    ///
    /// Panics if `u` is not in the range 0 to width-1, `v` is not in the range 0 to height-1, or
    /// both.
    pub fn get_texel_ints(&self, u: i32, v: i32) -> Rgb {
        if (u < 0) | (u >= self.width as i32) | (v < 0) | (v >= self.height as i32) {
            panic!("Tried to access a texel with an index outside the allowable range");
        }

        self.texels.get(u as usize + v as usize * self.width).unwrap().clone()
    }
    ...
    pub fn get_texel(&self, u: f64, v: f64, bilinear: bool) -> Rgb {
        if (u < 0.0) | (u > 1.0) | (v < 0.0) | (v > 1.0) {
            panic!("Tried to access a texel with a coordinate outside the range 0.0 to 1.0");
        }

        let u_exact = u * (self.width-1) as f64;
        let v_exact = v * (self.height-1) as f64;

        if bilinear {
            let u_fract = u_exact.fract();
            let u_left = i32::max(u_exact.floor() as i32, 0);
            let u_right = i32::min(u_exact.ceil() as i32, (self.width-1) as i32);

            let v_fract = v_exact.fract();
            let v_top = i32::max(v_exact.floor() as i32, 0);
            let v_bottom = i32::min(v_exact.ceil() as i32, (self.height-1) as i32);

            let top_left_color = self.get_texel_ints(u_left, v_top);
            let top_right_color = self.get_texel_ints(u_right, v_top);
            let bottom_left_color = self.get_texel_ints(u_left, v_bottom);
            let bottom_right_color = self.get_texel_ints(u_right, v_bottom);
            ...
            return self.get_texel_ints(u_exact.round() as i32, v_exact.round() as i32);
        }
    }
}
...
and bump mapping is one of: true false

The defaults are: both phong model perspective bilinear true

Bump mapping has no effect unless shading is Phong.
"#, cmd_name)
}
...
fn parse_command_line_arguments() -> Result<UserChoices, ()>{
    ...
    for arg in args {
        match arg.to_lowercase().as_str() {
            ...
            "true" => arg_bump_map = true,
            "false" => arg_bump_map = false,
    ...
    Result::Ok(UserChoices {
        lighting: arg_lighting,
        shading: arg_shading,
        normals: arg_normals,
        texture_perspective: arg_depth,
        texture_mapping: arg_texture_map,
        bump_mapping: arg_bump_map,
    })
}
            ...
            if depth_buffer.check_set_nearer_pixel(x as i32, y as i32, inv_z) {
                ...
                match user_choices.texture_perspective {
                    TexturePerspective::Linear => {
                        ...
                        bump_normal = triangle.normal_map.get_texel(
                            tuscan[x_minus_xl].1,
                            tvscan[x_minus_xl].1,
                            user_choices.texture_mapping == TextureMapping::Bilinear
                        );
                ...
                match user_choices.shading {
                    ...
                    Shading::Phong => {
                        ...
                        let normal;
                        if user_choices.bump_mapping {

                            // Convert bump normal adjustments to `x` and `y` coordinates to range
                            // -1.0 to 1.0. `green` is inverted as source uses OpenGL standard but
                            // our code uses DirectX standard. `z` coordinate is converted to range
                            // 0.0 to 1.0.
                            let bumped_normal = Vector3::new(
                                basic_normal.x * (bump_normal.red - 128.0) / 128.0,
                                basic_normal.y * (bump_normal.green - 128.0) / -128.0,
                                basic_normal.z * bump_normal.blue / 256.0,
                            ).normalize().unwrap();

                            normal = Vector4::from_vector3(&bumped_normal, 0.0);

//                             print!("{:?}\t", normal);

                        } else {
                            normal = Vector4::from_vector3(&basic_normal, 0.0);
                        }
...
fn main() {
    ...
    let mut canvas = Canvas::new("Raster 12 plus bilinear filtering (chapter 14) and normal mapping
        (chapter 15)", CANVAS_WIDTH, CANVAS_HEIGHT);
    let mut depth_buffer = DepthBuffer::new();

    let tex_image = Texture::new_from_rgb24_file(&TEXTURE_FILENAME);
    let tex_normals = Texture::new_from_rgb24_file(&TEXTURE_NORMAL_FILENAME);
    ...
    let instances = [
        ModelInstance::new(
            &cube,
            Vector4::new(1.0, 2.5, 7.5, 1.0),
            Matrix4x4::new_oy_rotation_matrix(210.0),
            2.0,
        ),
    ];
    ...
    let lights = [
        Light::Ambient(AmbientLightEntity { intensity: 0.3, } ),
        Light::Directional(DirectionalLightEntity {
            intensity: 1.0,
            vector: Vector4::new(0.0, 0.0, -1.0, 0.0),
        } ),
