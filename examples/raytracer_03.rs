//! Implementation of pseudocode from the second part of chapter 3 of Gabriel Gambetta's
//! [Computer Graphics from Scratch](https://gabrielgambetta.com/computer-graphics-from-scratch/) book.
//! It adds specular lighting to the scene.
//!
    ...
    for scene_ent in scene.entities.iter() {
        ...
        if let SceneEntity::Light(light_type) = scene_ent {
            if let LightType::Ambient(am_light) = light_type {
                ...
                if specular != -1 {
                    let reflection = normal.multiply_by(2.0)
                        .multiply_by(normal.dot(&light_vector))
                        .subtract(&light_vector);

                    let reflection_dot_towards_camera = reflection.dot(towards_camera);
                    if reflection_dot_towards_camera > 0.0 {
                        i += light_intensity * f64::powi(reflection_dot_towards_camera /
                                (reflection.length() * towards_camera.length()), specular);
                    }
                }
            }
        }
...
fn main() {
    let mut canvas = Canvas::new("Raytracer 03 (from the second part of chapter 3)", CANVAS_WIDTH, CANVAS_HEIGHT);
