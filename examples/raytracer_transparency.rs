...
//! [Computer Graphics from Scratch](https://gabrielgambetta.com/computer-graphics-from-scratch/) book with:
...
fn trace_ray(origin: &Vector3,
    ...
    if let Some(s) = closest_sphere {
        ...
        let color_with_reflection = local_color.multiply_by(1.0 - s.reflective).add(&(&reflected_color)
        ...
        if s.opacity == 1.0 {
            return color_with_reflection;
        } else {


            // TODO At this point we are on the boundary of a transparent sphere and wish to trace a path *through*
            //      to the other side, and then continue the ray until it hits a different sphere. Unsure how to do
            //      this, but the following is incorrect.

            let transparent_color = trace_ray(&position, &direction, TRACE_EPSILON, f64::INFINITY, 0, &scene);

            let col: Rgb = color_with_reflection.multiply_by(s.opacity)
                           .add(&transparent_color.multiply_by(1.0 - s.opacity)); // TODO plus light passing through transparent object
            return col;
        }
...
fn create_scene() -> Scene {
    Scene {
        ...
        entities: vec!(
            ...
            SceneEntity::Sphere(SphereEntity{
                center: Vector3::new(2.0, 0.0, 2.1),
                radius: 0.23,
                color: Rgb::from_ints(0, 255, 255),  // Cyan
                specular: 100,  // A bit shiny
                reflective: 0.01, // Unreflective
                opacity: 0.4,   // Quite transparent
            }),
