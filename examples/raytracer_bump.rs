...
fn trace_ray(origin: &Vector3,
    ...
    if let Some(s) = closest_sphere {
        ...
        let mut normal = position.subtract(&s.center);  // Compute normal sphere at this intersection

        // BUG Panics if light is in same location as sphere surface
        let mut normal_norm = normal.normalize().unwrap();

        // Basic bump mapping
        let x = Vector3::new(1.0, 0.0, 0.0).dot(&normal_norm);
        let bump_x = (x * 7.0).fract().abs();

        let y = Vector3::new(0.0, 1.0, 0.0).dot(&normal_norm);
        let bump_y = (y * 13.0).fract().abs();

        let z = Vector3::new(0.0, 0.0, 1.0).dot(&normal_norm);
        let bump_z = (z * 11.0).fract().abs();

        normal = normal.add(&Vector3::new(bump_x/3.1, bump_y/4.3, bump_z/3.1));
        normal_norm = normal.normalize().unwrap();
...
fn main() {
    ...
    let mut canvas = Canvas::new("Chapter 6 plus basic bump mapping", CANVAS_WIDTH,
                                 CANVAS_HEIGHT);
