...
//! book. This code implements several shading and lighting algorithms which can be chosen using
...
/// Returns a `Result` containing a 3-tuple of lighting, shading and normals options either chosen
/// by the user or not specified and at their default value. If the "help" option is chosen, or
/// any arguments are unrecognized, a help message listing usage information is shown and an `Err`
/// result is returned.
fn parse_command_line_arguments() -> Result<(Lighting, Shading, Normals), ()>{
    ...
    Result::Ok((arg_lighting, arg_shading, arg_normals))
}
    ...
    for d in 0..=divs {
        ...
        for d in 0..divs {
            for i in 0..divs-1 {
                ...
                triangles.push(Triangle::new((i0, i0+(divs as usize)+1, i0+1), *color)); // TODO Add normal
                triangles.push(Triangle::new((i0, i0+(divs as usize), i0+(divs as usize)+1), *color)); // TODO Add normal
            }
        }
    }
...
fn main() {
    if let Result::Ok((arg_lighting, arg_shading, arg_normals)) = parse_command_line_arguments() {
        println!("Arguments read successfully");
        println!("\tLighting = {:#?}", arg_lighting);
        println!("\tShading = {:#?}", arg_shading);
        println!("\tNormals = {:#?}", arg_normals);
    } else {
        return;
    }
