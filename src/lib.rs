//! A crate providing basic functionality to display a window with an image
//! constructed by setting pixel values one at a time. It is intended purely for
//! learning purposes, and in particular for people implementing the example
//! exercises in Gabriel Gambetta's
...
pub mod canvas {
    ...
    /// A basic wrapper around the `minifb` crate that creates a window with a
    /// black background. Individual pixels can be set one at a time using
    /// `put_pixel`, but changes are not shown until `display_until_exit` is
    /// called. `display_until_exit` does not return until the user exits
    /// the program, so no further changes can be made. This basic
    /// functionality is meant purely as a learning aid.
    ///
    /// # Examples
    ///
    /// ```
    ...
    impl Canvas {
        /// Creates a new window with a title of `name` and with the usable area
        /// (i.e., excluding window title bar and decorations), of the
        /// given `width` and `height`. The window is set
        /// as non-resizable to avoid the complexity of changing the usable
        /// canvas area in response to user actions.
        ...
        /// Sets a single pixel on the canvas at the given `x`, `y` coordinates.
        /// The center of the canvas is the origin, i.e., where `x = 0,
        /// y = 0`. `x` is the horizontal component ranging from
        /// `-width/2` at the furthest left to `width/2 - 1` at the
        /// furthest right.
        /// `y` is the vertical component, ranging from `-height/2` at the
        /// bottom to `height/2 - 1` at the top. If either `x` or `y` is
        /// outside these ranges, the function returns without setting a pixel.
        /// The pixel `color` is an [`Rgb` struct](struct.Rgb.html) defining
        /// red, green and blue components. Changes will only become
        /// visible when [`display_until_exit`](#method.display_until_exit) is
        /// called.
        ...
        ///  Updates the window with all pixels set using `put_pixel` and
        /// displays this by looping continuously until  the window
        /// closes or the user presses the escape key. This function does not
        /// return until either event  occurs, which means this function
    ...
    /// An RGB color expressed as red, green and blue components stored as
    /// floats and with no limits on their ranges. This is useful during
    /// calculations when values may be outside the normal RGB 0-255 range. Use
    /// [Rgb::clamp] to bring all components into this range.
    ///
    /// # Examples
    ///
    /// ```
